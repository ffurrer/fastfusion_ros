...
//////////////////////////////////////////////////////////////////////////////////////////////////////
// 25.01.2016: karrerm
inline void transformLoopSimPrecalculatedNegPointCloud_subtree
(
		const float *qxp1, const float *qxp2, const float *qxp3,
		const float *qyp1, const float *qyp2, const float *qyp3,
		const float &t1, const float &t2, const float &t3,
...
)
{
	...
	for(int y=0;y<_imageHeight;y++){
		int vxl = -1; int vyl = -1; int vzl = -1;
		for(int x=0;x<_imageWidth;x+=4){
			const int ptIndex = (y*_imageWidth+x);
			const float pz = depthData(y*_imageWidth+x);
			const float *noise = noiseData + (y*_imageWidth + x);
			int vx[4] = {
					(int)((qxp1[ptIndex  ] + qyp1[ptIndex  ]) + t1),
					(int)((qxp1[ptIndex+1] + qyp1[ptIndex+1]) + t1),
					(int)((qxp1[ptIndex+2] + qyp1[ptIndex+1]) + t1),
					(int)((qxp1[ptIndex+3] + qyp1[ptIndex+1]) + t1)
			};
			int vy[4] = {
					(int)((qxp2[ptIndex  ] + qyp2[ptIndex  ]) + t2),
					(int)((qxp2[ptIndex+1] + qyp2[ptIndex+1]) + t2),
					(int)((qxp2[ptIndex+2] + qyp2[ptIndex+2]) + t2),
					(int)((qxp2[ptIndex+3] + qyp2[ptIndex+3]) + t2)
			};
			int vz[4] = {
					(int)((qxp3[ptIndex  ] + qyp3[ptIndex  ]) + t3),
					(int)((qxp3[ptIndex+1] + qyp3[ptIndex+1]) + t3),
					(int)((qxp3[ptIndex+1] + qyp3[ptIndex+2]) + t3),
					(int)((qxp3[ptIndex+1] + qyp3[ptIndex+3]) + t3)
			};

			for(unsigned int k=0;k<4;k++){
				int xk = x+k;
				if(((pz[k])>0.0f && (pz[k])<maxcamdistance)
						&& ( (vx[k]&(-8)) != vxt[xk] || (vy[k]&(-8)) != vyt[xk] || (vz[k]&(-8)) != vzt[xk])
						&& ( (vx[k]&(-8)) != vxl || (vy[k]&(-8)) != vyl || (vz[k]&(-8)) != vzl)
				){
					vxt[xk] = vxl = vx[k]&(-8); vyt[xk] = vyl = vy[k]&(-8); vzt[xk] = vzl = vz[k]&(-8);
